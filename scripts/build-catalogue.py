...
head='''// Late-antique places, c. 600 CE. Cards use historical political identities; modern geography is reference metadata only.
// Every numerical stat and rarity is an authored, deterministic gameplay value.
export const MODERN_COUNTRIES={it:'Italy',fr:'France',es:'Spain',pt:'Portugal',be:'Belgium',nl:'Netherlands',de:'Germany',ch:'Switzerland',at:'Austria',hr:'Croatia',si:'Slovenia',rs:'Serbia',bg:'Bulgaria',gr:'Greece',al:'Albania',tr:'Türkiye',ro:'Romania'};
export const COUNTRIES=MODERN_COUNTRIES;
...
const mapRealm=realm=>realm==='Eastern Roman Empire'?'Eastern Roman Empire':realm==='Frankish Kingdom'||realm==='Duchy of Bavaria'?'Frankish Kingdom':realm==='Visigothic Kingdom'?'Visigothic Kingdom':['Lombard Kingdom','Duchy of Benevento','Duchy of Spoleto','Duchy of Friuli'].includes(realm)?'Lombard principalities':realm==='Avar Khaganate'?'Avars':realm;
export const CITIES=SEEDS.map(([id,name,modern,countryCode,lon,lat,rarity,realm,article],index)=>{
 ...
 const city={id,name,modern,countryCode,modernCountry:MODERN_COUNTRIES[countryCode],country:realm,region:'Europe',lon,lat,rarity,realm,mapRealm:mapRealm(realm),article,index,
  ...
  description:`${name}${name!==modern?` (present-day ${modern})`:''} is a late-antique settlement in the area of modern ${MODERN_COUNTRIES[countryCode]}. Around 600 CE its political context was ${realm}.`};
