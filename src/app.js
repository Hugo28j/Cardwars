...
import {WorldMap} from './map.js?v=20260926-fog-army-bounds-v5';
...
function battleDialogHTML1300(game,b){
 ...
 return `<div class="battle-dialog">
  ...
  <div class="battle-summary-grid"><span>Commander<strong>${esc(commander?.name||'No commander')}</strong></span><span>Your losses<strong class="negative">${strengthNumber(b.playerCasualties)}</strong></span><span>Enemy losses<strong>${strengthNumber(b.enemyCasualties)}</strong></span><span>Status<strong>${esc(b.status.toUpperCase())}</strong></span></div>
  ...
  ${b.status==='active'?`<div class="battle-retreat"><button data-action="battle-retreat" data-battle="${b.id}" ${canRetreat?'':'disabled'}>RETREAT</button><small>${canRetreat?'Withdraw to '+esc(displayCityName1300(CITY_1300[b.retreatCityId]||CITY_1300[b.armyHomeId]))+' · retreat may cause extra casualties':'Retreat becomes available after 2 battle days.'}</small></div>`:`<div class="battle-result ${b.status}"><strong>${b.status==='won'?'VICTORY':b.status==='lost'?'DEFEAT':'ARMY WITHDREW'}</strong><small>This battle is finished. Province occupation and peace terms will be added with the later war-score system.</small></div>`}
...
function provinceMilitaryHTML1300(game,c){
 ...
 return `<section class="province-military-panel">
  ...
  ${battle?`<button class="active-battle-banner" data-action="battle-open" data-battle="${battle.id}"><span>ACTIVE BATTLE</span><strong>${esc(displayCityName1300(CITY_1300[battle.cityId]))}</strong><small>Open tactics, casualties and retreat</small></button>`:''}
...
function gameProvincePanelHTML(cityId){
 ...
 <div class="province-side-scroll">
  ...
  ${gameProvinceTab==='economy'?`${owned?`<div class="workforce-summary province-economy-summary"><span><strong>${strengthNumber(summary.workers)}</strong><small>EMPLOYED</small></span><span><strong>${strengthNumber(summary.labour)}</strong><small>WORKER POOL</small></span><span><strong class="${summary.profit<0?'negative':''}">${summary.profit>=0?'+':'-'}ƒ${money1300(Math.abs(summary.profit))}</strong><small>PROFIT / WEEK</small></span></div>`:''}${owned?provinceMarketHTML1300(game,c.id):''}
