...
import {TECH_BRANCHES_1300,TECHNOLOGIES_1300,TECHNOLOGY_1300,freshTechnologyState1300,normaliseTechnologyState1300,technologyAvailable1300,technologyResearchCost1300,technologyBonuses1300,branchUnlockedCount1300,applyWeeklyResearch1300} from './technology1300.js?v=20260925-production-methods-v1';
...
const PRODUCTION_METHODS_1300={
 fields:[
  {id:'hand-cultivation',name:'Hand Cultivation',inputs:{},outputs:{grain:20},description:'Uses labour and basic hand work only. No purchased Tools input.'},
  {id:'tool-assisted-farming',name:'Tool-Assisted Farming',requiresTech:'heavy-tools',inputs:{tools:.4},outputs:{grain:30},description:'Heavy agricultural tools raise Grain output but create ongoing Tools costs.'}
 ],
 pastures:[
  {id:'open-herding',name:'Open Herding',inputs:{},outputs:{meat:6,wool:10},description:'Traditional herding with no purchased industrial input.'},
 ...
 textiles:[
  {id:'hand-looms',name:'Hand Looms',inputs:{wool:10},outputs:{cloth:10},description:'Traditional hand looms use Wool but no purchased Tools.'},
 ...
 fishery:[
  ...
  {id:'equipped-fishing',name:'Equipped Fishing',requiresTech:'improved-shipwrights',inputs:{wood:.6,salt:.5},outputs:{fish:22},description:'Improved boats and preservation supplies allow larger catches.'}
 ],
 forge:[
...
function productionMethodsForBuilding1300(buildingId){
 ...
 return [{id:'standard',name:'Standard Method',inputs:def.inputs||{},outputs:def.outputs||{},description:'The established production method for this company.'}];
}
...
function simulateWeeklyEconomy1300({cities=[],previousMarkets={},previousPops={},taxRate=10,taxCollectionFactor=.35,tariffs={},tradeStockpile={}}={}){
 ...
 for(const city of cities){
  ...
  for(const sector of city.sectors||[]){
   ...
   const directFlorins=Math.max(0,Number(p.def.directFlorins)||0)*scale,weeklyRevenue=revenueValue*FLORINS_PER_MARKET_VALUE+directFlorins,weeklyInputCost=inputValue*FLORINS_PER_MARKET_VALUE,annualWage=Math.max(.01,Number(sector.wage)||.01),fullWeeklyWageCost=p.workers*(annualWage/365)*7,weeklyWageCost=treasury<=0?0:fullWeeklyWageCost,unpaidWages=Math.max(0,fullWeeklyWageCost-weeklyWageCost),weeklyProfit=weeklyRevenue-weeklyInputCost-weeklyWageCost,weeklySectorTax=Math.max(0,weeklyProfit)*(clamp(Number(taxRate)||0,0,100)/100)*taxCollectionFactor;weeklyTax+=weeklySectorTax;
...
const GAME_WAGE_MIN=.02,GAME_WAGE_MAX=1.00,GAME_WAGE_STEP=.02,GAME_TAX_MIN=0,GAME_TAX_MAX=30,GAME_TAX_COLLECTION_FACTOR=.35;
...
function normaliseGameEconomy1300(raw){
 ...
 for(const [cityId,policies] of Object.entries({...e.companyPolicies})){
  ...
  for(const [buildingId,raw] of Object.entries({...policies})){
   ...
   policies[buildingId]={employmentTarget:clamp1300(Math.round(Number.isFinite(Number(raw.employmentTarget))?Number(raw.employmentTarget):100),0,100),recruitmentSupport:clamp1300(Math.round((Number(raw.recruitmentSupport)||0)*10)/10,0,2),priority,productionMethod};
  }
 }
...
function stabilityPolicyPressure1300(game){
 const e=normaliseGameEconomy1300(game.economy),wageRatio=e.nationalWage/expectedAnnualWage1300(game),tariffCost=tariffCostOfLivingImpact1300(game);
...
function applyLiveDynamicStats1300(game,factor=1/30){
 ...
 for(const id of game.ownedCities||[]){
  ...
  const groups=pop?.groups||[],popN=groups.reduce((n,g)=>n+(Number(g.size)||0),0),avgSol=popN?groups.reduce((n,g)=>n+(Number(g.standardOfLiving)||10)*(Number(g.size)||0),0)/popN:10,wageRatio=effectiveCityWage1300(game,id)/expectedAnnualWage1300(game),affordability=clamp1300((.72+.28*wageRatio+(avgSol-10)*.018)/Math.max(.65,priceRatio),.45,1.45),varietyBonus=(variety-1)*.035;
...
function companyPolicy1300(game,cityId,buildingId){
 ...
 const methods=productionMethodsForBuilding1300(buildingId),fallback=methods[0]?.id||'standard',productionMethod=methods.some(x=>x.id===raw.productionMethod)?raw.productionMethod:fallback;
 return {employmentTarget:clamp1300(Math.round(Number.isFinite(Number(raw.employmentTarget))?Number(raw.employmentTarget):100),0,100),recruitmentSupport:clamp1300(Math.round((Number(raw.recruitmentSupport)||0)*10)/10,0,2),priority:['employment','profit','output'].includes(raw.priority)?raw.priority:'employment',productionMethod};
}
...
function setCompanyPolicy1300(cityId,buildingId,patch={}){
 ...
 next.employmentTarget=clamp1300(Math.round(Number(next.employmentTarget)||0),0,100);next.recruitmentSupport=clamp1300(Math.round((Number(next.recruitmentSupport)||0)*10)/10,0,2);next.priority=['employment','profit','output'].includes(next.priority)?next.priority:'employment';const method=resolvedProductionMethod1300(game,buildingId,next.productionMethod);next.productionMethod=method.id;
...
function simulateGameEconomyDay1300(game,{forceMarket=false,collectRevenue=true}={}){
 ...
 for(const cityId of game.ownedCities||[]){
  ...
  const liveStats=provinceDynamicStats1300(game,c),sectors=state.buildings.filter(row=>row.level>0&&row.id!=='walls').map(row=>{const capacity=Math.max(1,row.maxWorkers*row.level),treasury=companyTreasury1300(game,cityId,row.id),wage=companyOperatingWage1300(game,c,row),wageRatio=wage/expectedAnnualWage1300(game),last=e.lastEconomy?.[cityId]?.[row.id],policy=companyPolicy1300(game,cityId,row.id),profitSignal=clamp1300((Number(last?.profit)||0)/8,-.35,.55),supportBoost=policy.recruitmentSupport*.14,priorityAttract=policy.priority==='employment'?.14:policy.priority==='output'?.06:Math.min(0,profitSignal*.12),attract=clamp1300(.18+.72*wageRatio+profitSignal+supportBoost+priorityAttract,.04,1.2),local=clamp1300(.80+(liveStats.stability-50)/300+(liveStats.economy-50)/500,.65,1.08),taxDrag=clamp1300(1-Math.max(0,e.taxRate-10)*.004,.80,1.03),targetCap=capacity*(policy.employmentTarget/100),profitRestraint=policy.priority==='profit'&&profitSignal<0?clamp1300(1+profitSignal,.55,1):1,treasuryCap=treasury<=0?.05:treasury<50?.20+.45*(treasury/50):1,desired=Math.round(Math.min(targetCap,capacity*clamp1300(attract*local*taxDrag*profitRestraint,.02,1)*treasuryCap));return {row,capacity,treasury,wage,policy,desired,score:wageRatio+profitSignal+supportBoost+(policy.priority==='employment'?.12:policy.priority==='output'?.05:0)};}).sort((a,b)=>b.score-a.score);
  let remaining=labour;for(const sec of sectors){const target=Math.min(sec.desired,remaining);remaining-=target;const current=Math.max(0,Number(e.employment[cityId][sec.row.id])||0),speed=1+sec.policy.recruitmentSupport*.60+(sec.policy.priority==='employment'?.35:sec.policy.priority==='output'?.15:0),distressExit=sec.treasury<=0?.45:sec.treasury<50?.25:0,move=Math.max(5,Math.round(sec.capacity*Math.max(.08*speed,distressExit)));e.employment[cityId][sec.row.id]=Math.round(current+clamp1300(target-current,-move,move));}
 }
...
function gameSectorMetrics1300(game,cityId,buildingId){
 ...
 return {workers,capacity,wage:effectiveBuildingWage1300(game,cityId,buildingId),treasury:companyTreasury1300(game,cityId,buildingId),gross:0,inputCost:0,wageBill:0,unpaidWages:0,profit:0,tax:0,throughput:0,inputs:{},outputs:{}};
}
...
function gameBuildingDetailHTML1300(cityId,buildingId){
 ...
 const coastal=isCoastalCity1300(c),e=game.economy=normaliseGameEconomy1300(game.economy),maxed=row.level>=ECONOMY_1300.maxBuildingLevel,blocked=row.requiresCoast&&!coastal,unavailable=!row.available&&row.level===0,pending=!!row.construction,canBuy=owned&&!pending&&!maxed&&!blocked&&!unavailable&&game.florins>=row.cost,m=owned?gameSectorMetrics1300(game,c.id,row.id):null,override=owned&&Number.isFinite(Number(e.buildingWages?.[c.id]?.[row.id])),effectiveWage=owned?effectiveBuildingWage1300(game,c.id,row.id):0,policy=owned?companyPolicy1300(game,c.id,row.id):null,productionMethods=owned?productionMethodsForBuilding1300(row.id):[],activeMethod=owned?resolvedProductionMethod1300(game,row.id,policy.productionMethod):null,buttonText=!owned?'FOREIGN':pending?'CONSTRUCTING':unavailable?'UNAVAILABLE':maxed?'MAX LEVEL':blocked?'NEEDS PORT':row.level?'UPGRADE':'BUILD',constructionPct=pending?constructionProgress1300(game,row.construction):0;
 ...
 return `<section class="building-detail-view">
  ...
  <div class="building-detail-section building-detail-wage-section"><div class="building-detail-wage"><div><span>Minimum wage</span><small>${override?'Custom wage':'Inherited wage'} · annual wage per worker</small></div>${wageStepper1300('building',c.id,row.id,effectiveWage,override)}</div></div>
...
function gameProvincePanelHTML(cityId){
 ...
 <div class="province-side-scroll">
  ...
  ${owned?`<section class="province-economic-policy"><div class="policy-heading"><span>PROVINCE POLICY</span><small>National tax and realm wage are set in the country Economy tab</small></div>
   <div class="policy-row"><div><strong>Province minimum wage</strong><small>${cityOverride?'Custom annual wage':'Inherits realm annual wage'} · per worker / year</small></div>${wageStepper1300('city',cityId,null,cityWage,cityOverride)}</div>
...
function countryPeople1300(game){
 const t=countryTotals1300(game),e=game.economy=normaliseGameEconomy1300(game.economy),simCities=(game.ownedCities||[]).map(id=>e.pops?.[id]).filter(Boolean),wageRatio=e.nationalWage/expectedAnnualWage1300(game),tariffCost=tariffCostOfLivingImpact1300(game),wageEffect=clamp1300((wageRatio-1)*16,-18,16),taxEffect=clamp1300(-(e.taxRate-10)*.70,-18,7),tariffEffect=clamp1300(-tariffCost*1.60,-30,0);
 if(simCities.length){
...
function countryRebellionRows1300(game){
 ...
 return (game.ownedCities||[]).map(id=>CITY_1300[id]).filter(Boolean).map(c=>{
  const wage=effectiveCityWage1300(game,c.id),stability=effectiveProvinceStability1300(game,c,gameProvinceBuildingState(c).bonuses.stability),risk=Math.round(clamp1300(100-stability+(e.taxRate-10)*.35-(wage/expectedAnnualWage1300(game)-1)*10+e.corruption*.12,0,100));
  return {c,risk};
...
function countryEconomyHTML1300(game){
 ...
 <section class="country-econ-policy"><div><span>National minimum wage</span><small>Annual wage per worker · higher wages can improve food affordability but reduce company profit</small></div>${wageStepper1300('national','',null,e.nationalWage,false)}</section>
