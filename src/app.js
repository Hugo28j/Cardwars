...
function weeklyDiplomaticPointGain1300(game){const player=buildCampaignRankings1300(game).find(r=>r.player);return round(Math.max(0,Number(player?.strength)||0)*.003/4,4);}
...
function settleGameWeek1300(game){
 ...
 game.florins=Math.max(0,Math.round((Number(game.florins)+balance)*100)/100);game.diploPoints=round(Math.max(0,Number(game.diploPoints)||0)+weeklyDiplomaticPointGain1300(game),4);applyWeeklyStabilityPolicy1300(game);applyWeeklyPopulationChange1300(game);settleWeeklyResearch1300(game);initializeDiplomacyWorld1300(game);for(const event of weeklyDiplomacy(game,completedCampaignWeeks1300(game),Object.fromEntries(buildCampaignRankings1300(game).map(r=>[r.country,r.strength||1]))))diplomacyLog1300(game,'World',event);updateCampaignRankingSnapshot1300(game);e.weekSectorRevenue=0;e.weekTariffRevenue=0;e.monthRevenue=0;e.monthExpenses=weeklyStateExpenses1300(game).total;
}
...
function setProvinceTechnologyBudget1300(cityId,value){
 const g=profile.activeGame,c=CITY_1300[cityId];if(!g||!c||!g.ownedCities?.includes(cityId))return;const max=provinceTechnologyBudgetMax1300(c,game),v=clamp1300(roundStat1300(value),0,max);g.economy.technologyBudgets[cityId]=v;g.economy.monthExpenses=weeklyStateExpenses1300(g).total;invalidateWeeklyBudgetProjection1300(g);save();refreshWeeklyBudgetDOM1300(g);document.querySelectorAll('[data-tech-budget-value="'+cityId+'"]').forEach(el=>el.textContent='ƒ'+money1300(v));
}
...
function campaignResourceBarHTML1300(game){
 ...
 return `<section class="campaign-resource-bar" aria-label="Realm resources">
  ...
  <article class="campaign-resource-box"><span>DIPLO POINTS</span><strong id="campaign-resource-diplo">${Number(game.diploPoints||0).toFixed(2)}</strong><small id="campaign-resource-diplo-week">+${weeklyDiplomaticPointGain1300(game).toFixed(3)}/week</small></article>
...
function refreshCampaignResourceBar1300(game){
 ...
 set('#campaign-resource-treasury','ƒ'+money1300(game.florins));set('#campaign-resource-population',strengthNumber(totals.population));set('#campaign-resource-diplo',Number(game.diploPoints||0).toFixed(2));set('#campaign-resource-diplo-week','+'+weeklyDiplomaticPointGain1300(game).toFixed(3)+'/week');set('#campaign-resource-pro-army',strengthNumber(prof.army));set('#campaign-resource-unpro-army',strengthNumber(unprof.army));set('#campaign-resource-navy',strengthNumber(mil.navy));
