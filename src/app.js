...
import {WorldMap} from './map.js?v=20260923-restored-old-map-v1';
...
function blankGood(g,previous){
 ...
 return {goodId:g.id,supply:0,demand:0,need:0,bought:0,sold:0,stock,fulfilled:0,tradeProfit:0,tariffRate:0,tariffRevenue:0,importShare:0,price:start,consumerPrice:Number.isFinite(consumer)&&consumer>0?consumer:start,previousPrice:start,targetPrice:g.basePrice,basePrice:g.basePrice};
}
...
function applyTariffsToMarket1300(market,tariffs={}){
 ...
 for(const g of GOODS_1300){
  const row=market.goods[g.id],rate=g.category==='service'?0:clamp(Number(tariffs?.[g.id])||0,0,50),fulfilled=Math.max(0,Number(row.fulfilled)||0),bought=Math.max(0,Number(row.bought)||0),share=fulfilled>0?clamp(bought/fulfilled,0,1):0;
  row.tariffRate=rate;row.importShare=round(share,4);row.consumerPrice=round(row.price*(1+(rate/100)*share),4);row.tariffRevenue=round(bought*row.price*FLORINS_PER_MARKET_VALUE*(rate/100),4);revenue+=row.tariffRevenue;
 }
...
function simulateWeeklyEconomy1300({cities=[],previousMarkets={},previousPops={},taxRate=10,taxCollectionFactor=.35,tariffs={}}={}){
 ...
 for(const market of Object.values(markets)){updatePrices(market);settleMarketFlows1300(market);weeklyTariffRevenue+=applyTariffsToMarket1300(market,tariffs);}
 for(const city of cities){
  ...
  for(const sector of city.sectors||[]){
   if((Number(sector.level)||0)<=0)continue;const p=sectorPotential(sector,city),inputIds=Object.keys(p.def.inputs||{}),availability=inputIds.length?Math.min(...inputIds.map(id=>clamp((Number(market.goods[id].fulfilled)||0)/Math.max(market.goods[id].demand,1e-6),.15,1))):1,priority=sector.priority||'employment',priorityThroughput=priority==='output'?1.10:priority==='profit'?.94:1,throughput=clamp(p.employmentRatio*availability*infra.access*priorityThroughput,0,1.2),scale=p.level*throughput*(.86+clamp(Number(city.technology)||50,0,100)/500)*(1+Math.min(p.level*.01,.30));
   ...
   for(const [id,n] of Object.entries(p.def.inputs||{})){const q=n*scale;inputs[id]=round(q,3);inputValue+=q*(Number(market.goods[id].consumerPrice)||market.goods[id].price);}
...
function normaliseGameEconomy1300(raw){
 ...
 for(const key of ['tariffs','cityWages','buildingWages','companyPolicies','employment','lastEconomy','markets','pops','dynamicStats','technologyBudgets','lastStatChanges','statRemainders'])if(!e[key]||typeof e[key]!=='object'||Array.isArray(e[key]))e[key]={};
...
function weeklyStateExpenses1300(game){
 const e=normaliseGameEconomy1300(game.economy),mil=militaryTotals1300(game),tech=(game.ownedCities||[]).reduce((n,id)=>n+(Number(e.technologyBudgets[id])||0),0),recruitmentSupport=companyRecruitmentSupportTotal1300(game);
...
function economyCitySnapshot1300(game,c){
 const state=gameProvinceBuildingState(c),e=game.economy,sectors=state.buildings.filter(row=>row.level>0).map(row=>{const policy=companyPolicy1300(game,c.id,row.id);return {id:row.id,level:row.level,workers:Math.max(0,Number(e.employment?.[c.id]?.[row.id])||0),capacity:Math.max(1,row.maxWorkers*row.level),wage:effectiveBuildingWage1300(game,c.id,row.id),employmentTarget:policy.employmentTarget,recruitmentSupport:policy.recruitmentSupport,priority:policy.priority};}),stats=provinceDynamicStats1300(game,c);
 return {id:c.id,population:Number(c.people)||0,food:stats.food,economy:stats.economy,technology:stats.technology,stability:stats.stability,coastal:isCoastalCity1300(c),labourPool:cityLabourPool1300(c),sectors};
}
function calculateWeeklyBudgetProjection1300(game){
 ...
 const result=simulateWeeklyEconomy1300({cities,previousMarkets:e.markets,previousPops:e.pops,taxRate:e.taxRate,taxCollectionFactor:GAME_TAX_COLLECTION_FACTOR,tariffs:e.tariffs}),expenses=weeklyStateExpenses1300(game),sectorTaxes=roundStat1300(result.weeklyTaxEstimate),importTariffs=roundStat1300(result.weeklyTariffRevenue),income=roundStat1300(sectorTaxes+importTariffs);
...
function simulateGameEconomyDay1300(game,{forceMarket=false,collectRevenue=true}={}){
 ...
 if(due){const cities=(game.ownedCities||[]).map(id=>CITY_1300[id]).filter(Boolean).map(c=>economyCitySnapshot1300(game,c)),result=simulateWeeklyEconomy1300({cities,previousMarkets:e.markets,previousPops:e.pops,taxRate:e.taxRate,taxCollectionFactor:GAME_TAX_COLLECTION_FACTOR,tariffs:e.tariffs});e.markets=result.markets;e.pops=result.pops;e.lastEconomy=result.sectorsByCity;e.weeklyTax=result.weeklyTaxEstimate;e.weeklyTariffRevenue=result.weeklyTariffRevenue;e.lastMarketTickDay=Number(game.day)||0;accrueTradeSurplus1300(game,result.markets);if(!forceMarket)applyLiveDynamicStats1300(game,1/WEEKS_PER_MONTH);if(collectRevenue){e.weekSectorRevenue=Math.round((Number(e.weekSectorRevenue||0)+result.weeklyTax)*100)/100;e.weekTariffRevenue=Math.round((Number(e.weekTariffRevenue||0)+result.weeklyTariffRevenue)*100)/100;e.monthRevenue=Math.round((e.weekSectorRevenue+e.weekTariffRevenue)*100)/100;}}
...
function gameBuildingDetailHTML1300(cityId,buildingId){
 ...
 const coastal=isCoastalCity1300(c),e=game.economy=normaliseGameEconomy1300(game.economy),maxed=row.level>=ECONOMY_1300.maxBuildingLevel,blocked=row.requiresCoast&&!coastal,unavailable=!row.available&&row.level===0,canBuy=owned&&!maxed&&!blocked&&!unavailable&&game.florins>=row.cost,m=owned?gameSectorMetrics1300(game,c.id,row.id):null,override=owned&&Number.isFinite(Number(e.buildingWages?.[c.id]?.[row.id])),effectiveWage=owned?effectiveBuildingWage1300(game,c.id,row.id):0,policy=owned?companyPolicy1300(game,c.id,row.id):null,buttonText=!owned?'FOREIGN':unavailable?'UNAVAILABLE':maxed?'MAX LEVEL':blocked?'NEEDS PORT':row.level?'UPGRADE':'BUILD';
 ...
 return `<section class="building-detail-view">
  ...
  <div class="building-detail-section company-control-section">
   ...
   <label class="company-control-priority"><span>Production priority</span><select data-company-priority data-city="${c.id}" data-id="${row.id}" aria-label="Production priority for ${esc(row.name)}"><option value="employment" ${policy.priority==='employment'?'selected':''}>Employment</option><option value="profit" ${policy.priority==='profit'?'selected':''}>Profit</option><option value="output" ${policy.priority==='output'?'selected':''}>Output</option></select><small>${policy.priority==='employment'?'Favors hiring and keeps more jobs filled.':policy.priority==='profit'?'Hiring is more cautious when the company is losing money.':'Pushes throughput and production higher.'}</small></label>
  </div>
...
function gameProvincePanelHTML(cityId){
 ...
 const summary=owned?gameCityEconomySummary1300(game,cityId):null,prof=owned?professionalArmyState1300(game):null,cityOverride=Number.isFinite(Number(e.cityWages[cityId])),cityWage=effectiveCityWage1300(game,cityId),techMax=provinceTechnologyBudgetMax1300(c),techBudget=Math.min(Number(e.technologyBudgets?.[cityId])||0,techMax),techNeed=technologyBudgetNeed1300(c);
 ...
 <div class="province-side-scroll">
  <section class="province-side-facts ${coastal?'has-navy':'no-navy'}"><div class="province-population-fact"><span>Population</span><strong>${esc(c.populationText||strengthNumber(c.people))}</strong></div>${coastal?`<div><span>Navy</span><strong>${strengthNumber(c.navy+b.navy)}</strong></div>`:''}<div><span>Professional army</span><strong>${strengthNumber(owned?(prof?.byCity[c.id]||0):c.army)}</strong></div><div><span>Unprofessional army</span><strong>${owned?'0':'—'}</strong></div></section>
...
function countryTechnologyHTML1300(game){
 ...
 return `<section class="tech-summary"><span>NATIONAL TECHNOLOGY</span><strong>${t.technology.toFixed(2)}<small>/${t.cap.toFixed(2)}</small></strong><p>Technology changes every Monday. Provincial investment is the main driver; every unlocked technology-tree milestone also gives every province a small research-efficiency bonus.</p><div class="tech-tree-bonus"><span>Unlocked milestones</span><strong>${unlocked}/${nodes.length}</strong><small>+${(unlocked*.008/WEEKS_PER_MONTH).toFixed(3)} technology/week before local investment effects</small></div></section>
...
document.addEventListener('click',async e=>{const b=e.target.closest('[data-action]');if(!b||b.disabled)return;const a=b.dataset.action,id=b.dataset.id;
 ...
 if(a==='quit-game'){profile.activeGame=null;gameScreen='map';gameProvincePanel=null;gameProvinceBuildingDetail=null;gameProvinceBuildingCatalog=false;gameCountryPanel=false;gameDiplomacyCountry=null;save();render();toast('You left the campaign. Your deck was kept.');return;}
