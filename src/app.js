...
import {TECH_BRANCHES_1300,TECHNOLOGIES_1300,TECHNOLOGY_1300,freshTechnologyState1300,normaliseTechnologyState1300,technologyAvailable1300,technologyResearchCost1300,technologyBonuses1300,branchUnlockedCount1300,applyWeeklyResearch1300} from './technology1300.js?v=20260925-full-tree-v1';
...
function technologyResearchIncome1300(game){
 ...
 const knowledgeBonus=1+Math.max(0,totals.technology)/1000,weekly=round(monthly*knowledgeBonus/WEEKS_PER_MONTH,2);return {weekly,monthly:round(monthly*knowledgeBonus,2),universities,monasteries,largeCities,knowledgeBonus};
}
...
function countryTechnologyHTML1300(game){
 ...
 return `<section class="tech-command"><div class="tech-command-stat"><span>NATIONAL TECHNOLOGY</span><strong>${totals.technology.toFixed(2)}<small>/${totals.cap.toFixed(2)}</small></strong><p>Every researched technology also permanently raises Technology in all your cities.</p></div><div class="research-ledger"><div><span>UNLOCKED</span><strong>${state.unlocked.length}<small>/25</small></strong></div><div><span>RESEARCH / WEEK</span><strong>+${income.weekly.toFixed(2)}</strong></div><div><span>UNSPENT</span><strong>${state.researchPoints.toFixed(1)}</strong></div></div>${active?`<div class="active-research"><div><span>ACTIVE RESEARCH</span><strong>${esc(active.name)}</strong><small>${activeProgress.toFixed(1)} / ${activeCost} · ${Math.round(activeProgress/activeCost*100)}%${activeContext.diffusionDiscount?` · ${Math.round(activeContext.diffusionDiscount*100)}% neighbour diffusion`:''}${activeContext.tradeDiscount?' · 10% trade diffusion':''}</small></div><i><b style="width:${Math.min(100,activeProgress/activeCost*100)}%"></b></i></div>`:`<div class="active-research empty"><span>NO ACTIVE RESEARCH</span><p>Choose any available technology below. Research Points remain stored while you decide.</p></div>`}<div class="research-sources"><span>Research sources</span><p>${income.universities} University levels · ${income.monasteries} Monastery levels · ${income.largeCities} cities above 50K · ${Math.round((income.knowledgeBonus-1)*100)}% city-Technology bonus</p></div></section>
 <div class="country-section-title"><span>TECHNOLOGY TREE</span><small>Choose freely between five branches · the second Tier III route costs +30%</small></div>
 <section class="full-tech-tree">${TECH_BRANCHES_1300.map(branch=>{const nodes=TECHNOLOGIES_1300.filter(x=>x.branch===branch.id),count=branchUnlockedCount1300(state,branch.id),specialized=count>=3;const iconName={agriculture:'wheat',economy:'hammer',army:'army',navy:'navy',administration:'temple'}[branch.id];return `<section class="tech-branch ${branch.id}"><header>${icon(iconName)}<div><span>${count}/5 RESEARCHED</span><strong>${esc(branch.name)}</strong><small class="${specialized?'active':''}">${specialized?'ACTIVE · ':''}${esc(branch.specialization)} — ${esc(branch.specializationEffect)}</small></div></header><div class="tech-branch-path">${nodes.map(tech=>technologyNodeHTML1300(game,state,tech)).join('')}</div></section>`;}).join('')}</section>
