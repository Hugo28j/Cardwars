...
function battleDialogHTML1300(game,b){
 ...
 return `<div class="battle-dialog">
  ...
  ${b.status==='active'?`<div class="battle-retreat"><button data-action="battle-retreat" data-battle="${b.id}" ${canRetreat?'':'disabled'}>RETREAT</button><small>Automatically withdraw to the nearest friendly province. Withdrawal may cause extra casualties.</small></div>`:`<div class="battle-result ${b.status}"><strong>${b.status==='won'?'VICTORY':b.status==='lost'?'DEFEAT':'ARMY WITHDREW'}</strong><small>This battle is finished. Province occupation and peace terms will be added with the later war-score system.</small></div>`}
