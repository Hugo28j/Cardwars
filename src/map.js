import {CITIES_1300 as CITIES,CITY_1300 as CITY} from './data1300.js?v=20260920-iberia-territories3';
...
export class WorldMap{
 ...
 async load(fit){
  ...
  try{cache[mode]??=fetch(mode==='modern'?'assets/modern-atlas.json?v=20260920-iberia-territories3':'assets/atlas.json?v=20260920-iberia-territories3').then(r=>{if(!r.ok)throw new Error('Missing atlas');return r.json();});const atlas=await cache[mode];this.realmInfo=new Map(atlas.map(f=>[realmOf(f),f]));if(this.destroyed||this.mode!==mode)return;
