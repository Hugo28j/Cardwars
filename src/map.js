import {CITIES,CITY} from './data.js';
...
export class WorldMap{
 ...
 async load(fit){
  ...
  try{cache[mode]??=fetch(mode==='modern'?'assets/modern-atlas.json':'assets/atlas.json').then(r=>{if(!r.ok)throw new Error('Missing atlas');return r.json();});const atlas=await cache[mode];if(this.destroyed||this.mode!==mode)return;
   this.svg.querySelector('#land').innerHTML=atlas.map((f,i)=>f.outline?`<path class="empire-outline" d="${f.d}" fill="none" stroke="#d7c48b" stroke-opacity=".62" stroke-width="1.35" stroke-dasharray="6 4" vector-effect="non-scaling-stroke"><title>${esc(f.name)}</title></path>`:`<path class="territory ${f.detail?'detail-polity':''}" data-realm="${esc(realmOf(f))}" data-country="${esc(f.code||'')}" d="${f.d}" fill="${mode==='historical'?colorForRealm(realmOf(f)):palettes[i%palettes.length]}" fill-rule="evenodd" stroke="#22322b" stroke-width="${f.detail?'.55':'.8'}" vector-effect="non-scaling-stroke"><title>${esc(f.name||'Local communities')}</title></path>`).join('');
 ...
 refresh(){const unit=this.view.w/(this.host.clientWidth||1000),s=this.state,occupied=[];
  ...
  this.svg.querySelectorAll('.realm-label').forEach(t=>{const level=+(t.dataset.level||1),show=level===1||(level===2&&unit<.55)||(level===3&&unit<.30);t.style.display=show?'':'none';t.style.fontSize=Math.max(level===3?4.2:5,Math.min(level===1?15:10,unit*(level===1?12:8.5)))+'px';});
 }
