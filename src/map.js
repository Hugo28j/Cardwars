  ...
  if(militaryLayer){
   if(!showMilitary)militaryLayer.innerHTML='';
    ...
    for(const c of CITIES){
     ...
     const owned=ownedCities.has(c.id),override=game?.militaryByCity?.[c.id],battle=game?.battlesByCity?.[c.id]||null,siege=game?.siegesByCity?.[c.id]||null,occupied=!!game?.occupations?.[c.id],playerOverride=!!override&&Number(override.army)>0,relation=mapArmyRelation(game,c,playerOverride),fallbackArmy=battle||siege||occupied?0:c.army,army=Math.max(0,Number(override?.army??fallbackArmy)||0),navy=Math.max(0,Number(override?.navy??c.navy)||0);
     ...
     if(battle&&Number(battle.playerCount)>0&&Number(battle.enemyCount)>0&&sq.x>-55&&sq.y>-65&&sq.x<width+55&&sq.y<height+65){
      const safePx=Math.max(1,(Number(unitAnchor?.clearance)||0)/unit),horizontal=safePx>=46,scale=Math.max(.48,Math.min(.82,safePx/52)),offsets=horizontal?[[-22,4],[22,4]]:[[0,-27],[0,31]],playerPoint=[land[0]+offsets[0][0]*unit,land[1]+offsets[0][1]*unit],enemyPoint=[land[0]+offsets[1][0]*unit,land[1]+offsets[1][1]*unit],selectedArmy=game?.selectedArmyCity===c.id;
