import {CITIES_1300 as CITIES,CITY_1300 as CITY} from './data1300.js?v=20260920-iberia-territories2';
...
export class WorldMap{
 ...
 async load(fit){
  ...
  try{cache[mode]??=fetch(mode==='modern'?'assets/modern-atlas.json?v=20260920-iberia-territories2':'assets/atlas.json?v=20260920-iberia-territories2').then(r=>{if(!r.ok)throw new Error('Missing atlas');return r.json();});const atlas=await cache[mode];this.realmInfo=new Map(atlas.map(f=>[realmOf(f),f]));if(this.destroyed||this.mode!==mode)return;
 ...
 refresh(){const width=this.host.clientWidth||1000,height=this.host.clientHeight||600,unit=this.view.w/width,s=this.state,occupied=[];
  ...
  const cityAreaLabels=this.svg.querySelector('#city-territory-labels');
  if(cityAreaLabels){
   const showCityAreas=unit<.082;
   cityAreaLabels.innerHTML=showCityAreas?CITIES.filter(isIberianCity).map(c=>{
    const p=pos(c.mapLon??c.lon,c.mapLat??c.lat),q=screen(...p);
