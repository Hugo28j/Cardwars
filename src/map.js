...
export class WorldMap{
 ...
 async load(fit){
  ...
  try{cache[mode]??=fetch(mode==='modern'?'assets/modern-atlas.json?v=20260920-aquileia-venice-coast-fix-v1':'assets/atlas-1300-v2.json?v=20260923-continuous-v2').then(r=>{if(!r.ok)throw new Error('Missing atlas');return r.json();});physicalLandCache??=fetch('assets/map-land.json?v=20260920-aquileia-venice-coast-fix-v1').then(r=>r.ok?r.json():{d:''}).catch(()=>({d:''}));const [atlas,physicalLand]=await Promise.all([cache[mode],physicalLandCache]);this.physicalLandPolys=svgSubpaths(physicalLand.d).map(svgSubpathPoints).filter(p=>p.length>=3);this.coastMarkerCache.clear();this.realmInfo=new Map(atlas.map(f=>[realmOf(f),f]));if(this.destroyed||this.mode!==mode)return;
   ...
   this.host.querySelector('#map-attribution').textContent='Continuous generalized c. 1300 borders · Natural Earth coastline';this.host.querySelector('.map-loading')?.remove();if(fit&&this.focusRequest)this.focus(this.focusRequest);else if(fit)this.fit();else this.update();
