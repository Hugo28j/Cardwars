   ...
   if(!showMilitary)militaryLayer.innerHTML='';
    ...
    for(const c of CITIES){
     ...
     const owned=ownedCities.has(c.id),override=game?.militaryByCity?.[c.id],army=Math.max(0,Number(override?.army??c.army)||0),navy=Math.max(0,Number(override?.navy??c.navy)||0);
      ...
      const selectedArmy=game?.selectedArmyCity===c.id,marker=`<g class="army-map-marker ${selectedArmy?'selected-army':''} ${game?.armyMoveMode&&selectedArmy?'move-armed':''}" data-unit-city="${c.id}" transform="translate(${chosen.point}) scale(${unit*markerScale})"><title>${esc(displayCityName(c))}: ${compactMilitaryNumber(army)} soldiers</title><circle class="army-click-hitbox" cx="0" cy="0" r="28" fill="transparent" pointer-events="all"/>${soldierPiece(army,owned,badgeY)}</g>`;
