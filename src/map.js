...
export class WorldMap{
 constructor(host,state,onSelect,onRegion){
  ...
  host.innerHTML=`<svg id="world-map" role="img" aria-label="Political map of Europe around 1300 CE. Drag to pan, scroll or pinch to zoom, click a country or city to inspect it." tabindex="0"><rect x="-5000" y="-5000" width="15000" height="15000" fill="#192c32"/><g id="land"></g><path id="map-borders" fill="none" stroke="#344138" stroke-width=".8" stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke" pointer-events="none"/><path id="map-coast" fill="none" stroke="#243a35" stroke-width=".9" stroke-linejoin="round" vector-effect="non-scaling-stroke" pointer-events="none"/><g id="realm-labels"></g><g id="sea-labels"></g><g id="cities"></g></svg><div class="map-top"><div class="map-heading"><span class="eyebrow">EUROPE & ANATOLIA</span><span>${CITIES.length} researched city cards · political map c. 1300 CE</span></div><div class="map-era-badge">REALMS · c. 1300 CE</div></div><div class="map-bottom"><span class="map-hint">Drag to explore · Zoom in for smaller states · Click to inspect</span><span id="map-attribution" class="map-attribution">Generalized borders · c. 1300 CE · Historical Basemaps / Natural Earth</span><span class="map-key"><i></i> Researched 1300 city card</span></div><div class="map-controls"><button data-map="in" title="Zoom in" aria-label="Zoom in">${icon('plus')}</button><button data-map="out" title="Zoom out" aria-label="Zoom out">${icon('minus')}</button><button data-map="selected" title="Focus selected city" aria-label="Focus selected city">${icon('target')}</button><button data-map="all" title="Show map overview" aria-label="Show map overview">${icon('globe')}</button></div><div class="map-compass" aria-hidden="true"><span>N</span><i></i></div><div class="map-loading">Unfolding the atlas…</div>`;
 ...
 async load(fit){
  ...
  try{cache[mode]??=fetch('assets/atlas-1300-v2.json').then(r=>{if(!r.ok)throw new Error('Missing atlas');return r.json();});const atlas=await cache[mode];if(this.destroyed||this.mode!==mode)return;
   this.svg.querySelector('#land').innerHTML=atlas.features.map(f=>`<path class="territory ${f.detail?'detail-polity':''}" data-realm="${esc(realmOf(f))}" data-detail="${f.detail?'1':'0'}" d="${f.d}" fill="${palettes[f.paletteIndex]||colorForRealm(realmOf(f))}" fill-rule="evenodd" stroke="none"><title>${esc(f.name)}${f.note?' · '+esc(f.note):''}</title></path>`).join('');
   this.svg.querySelector('#map-borders').setAttribute('d',atlas.borders);
   this.svg.querySelector('#map-coast').setAttribute('d',atlas.land);
   this.svg.querySelector('#realm-labels').innerHTML=atlas.labels.map(({label,lx,ly,labelLevel=2,umbrella})=>{const p=pos(lx,ly);return `<text x="${p[0]}" y="${p[1]}" text-anchor="middle" data-level="${labelLevel}" data-kind="${umbrella?'umbrella':labelLevel===1?'major':'polity'}" class="realm-label">${esc(label)}</text>`;}).join('');
   ...
   this.host.querySelector('#map-attribution').textContent='Generalized borders · c. 1300 CE · Historical Basemaps / Natural Earth';this.host.querySelector('.map-loading')?.remove();if(fit&&this.focusRequest)this.focus(this.focusRequest);else if(fit)this.fit();else this.update();
