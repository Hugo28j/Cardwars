...
export class WorldMap{
 ...
 async load(fit){
  ...
  try{cache[mode]??=fetch(mode==='modern'?'assets/modern-atlas.json?v=20260920-aquileia-venice-coast-fix-v1':'assets/atlas.json?v=20260920-merge-frankfurt-into-mainz-v1').then(r=>{if(!r.ok)throw new Error('Missing atlas');return r.json();});physicalLandCache??=fetch('assets/map-land.json?v=20260920-aquileia-venice-coast-fix-v1').then(r=>r.ok?r.json():{d:''}).catch(()=>({d:''}));mapPaintCache??=fetch('assets/map-paint-import-20260923.json?v=20260923-test-v1').then(r=>r.ok?r.json():null).catch(()=>null);const [atlas,physicalLand,mapPaint]=await Promise.all([cache[mode],physicalLandCache,mode==='historical'?mapPaintCache:Promise.resolve(null)]);this.physicalLandPolys=svgSubpaths(physicalLand.d).map(svgSubpathPoints).filter(p=>p.length>=3);this.coastMarkerCache.clear();this.realmInfo=new Map(atlas.map(f=>[realmOf(f),f]));if(this.destroyed||this.mode!==mode)return;
   ...
   this.renderMapPaintOverlay(mapPaint);
 ...
 renderMapPaintOverlay(paint){
  const layer=this.svg?.querySelector('#map-paint-overlay');if(!layer)return;layer.innerHTML='';
  if(this.mode!=='historical'||!paint||!Array.isArray(paint.operations))return;
  const pathFor=points=>{if(!Array.isArray(points)||points.length<2)return '';return 'M'+points.map(p=>Number(p[0]).toFixed(3)+','+Number(p[1]).toFixed(3)).join('L');};
  let markup='';
  for(const op of paint.operations){
   const d=pathFor(op.points);if(!d)continue;
   if(op.type==='erase'){
    const fill=colorForRealm(op.realm||'');
    markup+=`<path d="${d}" fill="none" stroke="${fill}" stroke-width="8" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>`;
   }else if(op.type==='draw'){
    markup+=`<path d="${d}" fill="none" stroke="#07110e" stroke-opacity=".48" stroke-width="5.4" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/><path d="${d}" fill="none" stroke="#18241f" stroke-width="1.15" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke"/>`;
   }
  }
