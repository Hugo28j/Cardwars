...
test('technology tree has five complete branches and thirty unique technologies',()=>{
 assert.equal(TECHNOLOGIES_1300.length,30);
 assert.equal(new Set(TECHNOLOGIES_1300.map(x=>x.id)).size,30);
 for(const branch of ['agriculture','economy','army','navy','administration'])assert.equal(TECHNOLOGIES_1300.filter(x=>x.branch===branch).length,6);
});

test('requirements open both tier-three routes, which converge before the capstone',()=>{
 ...
 assert.equal(technologyAvailable1300(state,'manorial-granaries'),false);
 assert.equal(technologyAvailable1300(state,'agricultural-surplus'),false);
 state.unlocked.push('intensive-cultivation');
 assert.equal(technologyAvailable1300(state,'manorial-granaries'),true);
 assert.equal(technologyAvailable1300(state,'agricultural-surplus'),false);
 state.unlocked.push('manorial-granaries');
...
test('specialisation, alternate-route and diffusion modifiers affect research cost',()=>{
 ...
 assert.equal(technologyResearchCost1300(state,TECHNOLOGY_1300['agricultural-surplus'],{diffusionDiscount:.15,tradeDiscount:.10}),390);
});
